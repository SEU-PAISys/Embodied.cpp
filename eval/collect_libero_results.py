...
def collect_model(model_dir: Path, task_suite: str, n_tasks: int) -> dict | None:
...
def main() -> int:
    ...
    ap.add_argument("--suite", default="libero_object",
                    help="LIBERO suite to collect: libero_object, libero_goal, "
                         "libero_spatial, libero_10, libero_90, or libero_long "
                         "(default: libero_object)")
    ap.add_argument("--n-tasks", type=int, default=None,
                    help="number of tasks to scan (default: suite preset; libero_90=90)")
    ...
    args = ap.parse_args()
    task_suite = TASK_SUITE_ALIASES.get(args.suite, args.suite)
    if task_suite not in DEFAULT_TASK_COUNTS and args.n_tasks is None:
        print(f"ERROR: unknown suite '{args.suite}'. Pass --n-tasks to override.",
              file=sys.stderr)
        return 1
    n_tasks = args.n_tasks or DEFAULT_TASK_COUNTS.get(task_suite)
    if args.sweep == DEFAULT_SWEEP and task_suite != "libero_object":
        args.sweep = REPO_ROOT / "outputs" / f"{task_suite}_sweep"
    ...
    mem_stats: dict[str, dict] = {}
    for m in models:
        per_task = collect_model(m, task_suite, n_tasks)
        if per_task is None:
    ...
    if not args.no_md:
        ...
        md_path.write_text(
            render_markdown(args.sweep, task_suite, n_tasks, rows, server_stats,
                            mem_stats, per_task_all, repro_block),
            encoding="utf-8",
        )
        print()
