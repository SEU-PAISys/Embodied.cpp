...
# Run a LIBERO eval suite (N_EPISODES per task) against every VLA
...
usage() {
    cat <<EOF
Usage: $(basename "$0") -i <MODELS_ROOT> [-o <OUTPUT_ROOT>] [-n <N_EPISODES>] [-m <MODEL>] [-s SUITE] [-T TASK_IDS]
...
while getopts ":i:o:n:m:s:T:h" opt; do
    case "${opt}" in
