...
libmtmd chat runtime (`vlm/engine.cpp`) behind a ZMQ daemon. Send text and
...
The client needs `pyzmq` + `protobuf`. If `serving/vlm.proto` changes,
regenerate the binding next to the client:

```bash
protoc --proto_path=serving --python_out=examples/chat serving/vlm.proto
```
